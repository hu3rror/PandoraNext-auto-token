...
# get POOL_TOKEN and API_ENDPOINT from secrets
...
def process_users(users_file, session_tokens_file):
    ...
    if not lines:
        print_log(f"{users_file}: No users found. Exiting program.")
        return
    ...
    for line in lines:
        ...
        if response.status_code == 200:
            ...
            print_log(f"Failed for {username}: {error_detail}")

    write_tokens(session_tokens_file, session_tokens)
    num_tokens = len(session_tokens)
    print_log(
        f"Successfully extracted {num_tokens} session tokens, Saved to file {session_tokens_file}"
    )


def process_session_and_register(session_token):
    session_payload = f"session_token={session_token}"
    session_response = make_post_request(
        api_endpoint, "/api/auth/session", session_payload
    )

    if session_response.status_code != 200:
        error_message = session_response.json().get("detail")
        print_log(f"Failed session token: {error_message}")
        return None
    ...
    register_response = make_post_request(
        api_endpoint, "/api/token/register", register_payload
    )

    if register_response.status_code != 200:
        error_message = register_response.json().get("detail")
        print_log(f"Failed register: {error_message}")
        return None
...
def process_tokens(tokens_file, session_tokens_file):
    ...
    if not tokens:
        print_log(f"{session_tokens_file}: No tokens found. Exiting program.")
        return

    #write_tokens(f"{session_tokens_file}_bak", tokens)
    ...
    for token in tokens:
        ...
        if token_info:
            token_expire_dict[token_info["expire_at"]] = {
                token_info["share_token"]: token_info["session_token"]
            }
            share_tokens.append(token_info["share_token"])
            session_tokens.append(token_info["session_token"])
            print_log(f"Share Token: {token_info['share_token']}")
    ...
    if share_tokens:
        ...
        pool_response = make_post_request(
            api_endpoint, "/api/pool/update", pool_payload
        )

        if pool_response.status_code == 200:
            pool_info = pool_response.json()
            pool_token = pool_info["pool_token"]
            print_log(f"Count: {pool_info['count']} Pool Token: {pool_token}")

        write_tokens(tokens_file, share_tokens + [pool_token])
        print_log(f"Saved to file {tokens_file}")

    if session_tokens:
        write_tokens(session_tokens_file, session_tokens)
        print_log(f"Updated {session_tokens_file}")
    ...
    while True:
        ...
        if current_time > min_expire_time:
            ...
            if register_info:
                print_log(f"Updated Share Token: {register_info['share_token']}")
                token_expire_dict[register_info["expire_at"]] = {
                    register_info["share_token"]: register_info["session_token"]
                }
                session_tokens.append(register_info["session_token"])
            else:
                if share_token in share_tokens:
                    share_tokens.remove(share_token)
                    print_log(f"Expired. Deleted Share Token: {share_token}")

                if share_tokens:
                    share_token_str = "%0D%0A".join(share_tokens)
                    pool_payload = (
                        f"share_tokens={share_token_str}&pool_token={pool_token}"
                    )
                    pool_response = make_post_request(
                        api_endpoint, "/api/pool/update", pool_payload
                    )
                    ...
                    if pool_response.status_code == 200:
                        ...
                        pool_token = pool_info["pool_token"]
                        print_log(
                            f"Updated Pool Token. Count: {pool_info['count']} Pool Token: {pool_token}"
                        )

                write_tokens(tokens_file, share_tokens + [pool_token])
                print_log(f"Updated {tokens_file}")
            ...
            print_log(
                f"Next run at {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time() + time_to_wait))}"
            )
